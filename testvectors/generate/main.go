...
func main() {
	generateJSONTestVectorsForDynamicLinks()
}

type TestCase = testvectors.TestCase
...
func genAll(gp gp) (link, blobName, key, decrypted []byte) {
	...
	blobName = append([]byte{0x02}, hash[:]...)
...
func genLink(gp gp) []byte {
	//nolint:dogsled // helper function to get only a link
	link, _, _, _ := genAll(gp)
	return link
}

func blobName(gp gp) []byte {
	//nolint:dogsled // helper function to get only a blob name
	_, blobName, _, _ := genAll(gp)
	return blobName
}

func key(gp gp) []byte {
	//nolint:dogsled // helper function to get only a key
	_, _, key, _ := genAll(gp)
	return key
}

func decrypted(gp gp) []byte {
	//nolint:dogsled // helper function to get only a decrypted dataset
...
func writeLinkData(tc TestCase) {
	fName := "../data/" + tc.Name + ".json"
	...
	// Re-marshal through dynamic map to get alphabetic order of the keys
	...
	err = os.WriteFile(fName, jsonData, 0o600)
	cutl.PanicIfError(err)
}

func generateJSONTestVectorsForDynamicLinks() {
	...
	for i := range 10 {
		...
		writeLinkData(TestCase{
			...
			UpdateDataset:    genLink(params),
			BlobName:         blobName(params),
			EncryptionKey:    key(params),
			DecryptedDataset: decrypted(params),
			ValidPublicly:    true,
			ValidPrivately:   true,
		})
	}
